...
def test_ata_factors_calculation_benchmark(company_data):
    ...
    print(f"\nATA factors timing: Notebook looping={t_nb:.6f}s | Engine={t_engine:.6f}s")
    np.testing.assert_allclose(factors_nb.values, factors_engine.values, rtol=1e-7, equal_nan=True)

@pytest.mark.performance
def test_ldf_averages_benchmark(company_data):
    paid_masked = company_data
    factors_engine = DevelopmentEngine.calculate_ata_factors(paid_masked)
    ...
    print(f"LDF averages timing: Notebook looping={t_nb:.6f}s | Engine={t_engine:.6f}s")
...
@pytest.mark.performance
def test_cdfs_calculation_benchmark(company_data):
    paid_masked = company_data
    factors_engine = DevelopmentEngine.calculate_ata_factors(paid_masked)
    avg_simple = DevelopmentEngine.calculate_average_ldfs(factors_engine, method="simple")
    ...
    print(f"CDFs calculation timing: Notebook={t_nb:.6f}s | Engine={t_engine:.6f}s")
    np.testing.assert_allclose(cdfs_nb.values, cdfs_engine, rtol=1e-7)

@pytest.mark.performance
def test_ultimate_projection_benchmark(company_data):
    paid_masked = company_data
    factors_engine = DevelopmentEngine.calculate_ata_factors(paid_masked)
    avg_simple = DevelopmentEngine.calculate_average_ldfs(factors_engine, method="simple")
    cdfs_engine = DevelopmentEngine.calculate_cdfs(avg_simple.values, tail_factor=1.0)
    ...
    print(f"Ultimate projection timing: Notebook={t_nb:.6f}s | Engine={t_engine:.6f}s")
