...
class CaseOutstanding(MethodBase):
    ...
    def _compute(self):
        ...
        dev_idx = [next((i for i, v in reversed(list(enumerate(row))) if v is not None and not np.isnan(v)), 0) for row in (self.triangle.matrix if self.triangle.matrix else [])]
